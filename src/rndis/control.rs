...
use crate::rndis::wire::{self, InitComplete};
...
mod tests {
    ...
    fn bring_up_runs_init_query_set_in_order() {
        ...
        let types: Vec<u32> = sent.iter().map(|m| wire::u32_at(m, 0).unwrap()).collect();
    ...
    fn a_device_keepalive_is_answered_during_a_transaction() {
        ...
        let answered = sent.borrow().iter().any(|m| {
    ...
    fn media_disconnect_is_reported() {
        ...
        rndis.initialize().unwrap();
        assert_eq!(rndis.link_state(), LinkState::Down);
    }
