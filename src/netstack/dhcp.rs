...
use crate::netstack::ethernet::MacAddr;
...
mod tests {
    ...
    fn completes_the_four_way_exchange() {
        let (mut client, lease) = bind();
        assert!(
            client.retransmit().is_none(),
            "a bound client has nothing outstanding"
        );
    ...
    fn ignores_replies_for_another_transaction() {
        ...
        // Still selecting, so a retransmit repeats the DISCOVER.
        assert_eq!(
    ...
    fn a_nak_drops_the_lease() {
        ...
        // Back to square one: nothing left to retransmit.
        assert!(c.retransmit().is_none());
    }
